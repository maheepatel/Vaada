...
export const metadata: Metadata = {
  title: 'Rankings',
  description:
    'State and district rankings by how many promises were actually kept, plus every issue on the register.',
};
      ...
      <div className="mt-3 border-y py-2">
        <StatRibbon
          items={[
            ...
            { value: formatCount(live.reduce((s, c) => s + (c.beneficiaries ?? 0), 0)), label: 'people affected' },
          ]}
        />
      </div>

      <section className="mt-5">
        <h2 className="h-section display mb-2.5">States</h2>
        <LeagueTable rows={states} />
      </section>

      <section className="mt-8">
        <h2 className="h-section display mb-2.5">Districts</h2>
        <LeagueTable rows={districts} showState />
      </section>

      <p className="mt-3 max-w-3xl text-[0.75rem] leading-relaxed text-ink-3">
        <strong>*</strong> means fewer than {MIN_DECIDED} decided promises. The
        position is real, the sample is thin. State-wide commitments are excluded
      ...
      <section className="mt-14 grid gap-6 lg:grid-cols-2">
        <div>
          <SectionHeading eyebrow="Delivery speed" title="Who finishes early">
            How much of the agreed window had gone when the work was verified.
            Lower is faster.
          </SectionHeading>
          {fastest.length === 0 ? (
            <Empty title="Nothing kept and verified yet." />
          ) : (
            <Card className="overflow-hidden">
              <ul className="divide-y">
                {fastest.map((d, i) => (
                  <li
                    key={`${d.stateName}-${d.slug}`}
        ...
        <div>
          <SectionHeading eyebrow="By subject" title="What gets delivered">
            Which kinds of promise actually land.
          </SectionHeading>
          <Card className="overflow-hidden">
            <ul className="divide-y">
              {categories.map((c) => (
